Create a single horizontal sprite strip for the Codex app digital pet `moomoo` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: A compact chibi black-and-white cow digital pet based on the reference images: round oversized head and soft chunky body, black cow patches around ears and head, pale pink muzzle and cheeks, tiny nostrils, short limbs with dark hooves, stern expressive eyes with thick dark eyebrows and a small stubborn mouth. Preserve the funny defiant personality, but simplify into a cute Codex pet; no Chinese text, no scenery, no microphone, no drink cup, no flower pot, no readable labels..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex digital pet style: small pixel-art-adjacent mascot, compact chibi proportions, chunky readable silhouette, thick dark 1-2 px outline, stepped pixel edges, limited black white pink palette, flat cel shading, simple expressive face, tiny limbs, transparent final atlas..
